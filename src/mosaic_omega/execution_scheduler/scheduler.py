    ...
    def _candidates(
        ...
        for agent, model, tool, device in itertools.product(*groups):
            ...
            assignment = Assignment(
                ...
                reason=(f"eligible bundle on {device.actor_id}; cost={evaluation.total:.4f}; "
    ...
    def _ortools(self, tasks: list[TaskNodeView]) -> list[Assignment]:
        ...
        for arc in range(flow.num_arcs()):
            ...
            if tail in reverse_tasks and head in reverse_devices:
                ...
                raw["policy"] = "ortools"
                raw["solver_provenance"] = {
                    "engine": "ortools.graph.python.min_cost_flow.SimpleMinCostFlow",
                    "algorithm": "min_cost_flow",
                    "status": "OPTIMAL",
                    "target_flow": target_flow,
                    "available_capacity": available_capacity,
                    "objective_cost": flow.optimal_cost(),
                }
